...
// Authenticated fetch — stuurt automatisch API key mee
function authFetch(url, options = {}) {
  const key = (JSON.parse(localStorage.getItem("ccc-settings") || "{}")).apiKey || "";
  const headers = { ...(options.headers || {}) };
  if (key) { headers["X-CCC-Key"] = key; }
  else { console.warn("⚠️ CCC: Geen API key in localStorage! Ga naar Settings en vul je key in."); }
...
const api = {
  async log(action, detail, type = "action", source = "Dashboard", mac = "MBA") {
    try {
      await authFetch(`${WORKER_API}/api/log`, {
        method: "POST",
  ...
  async askAI(messages) {
    try {
      const r = await authFetch(`${WORKER_API}/api/ai`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async scrapeUrl(url) {
    try {
      const r = await authFetch(WORKER_API + "/api/scrape", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async getDump() {
    try {
      ...
      if (data.error) { console.error("Dump API error:", data.error, "— check API key in Settings"); return { items: [] }; }
      return data;
    } catch (e) { console.error("Get dump failed:", e); return { items: [] }; }
  },
  async saveDump(items, source) {
    try {
      await authFetch(WORKER_API + "/api/dump", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async semanticSearch(query) {
    try {
      var r = await authFetch(WORKER_API + "/api/semantic-search?q=" + encodeURIComponent(query));
      return await r.json();
    } catch (e) { console.error("Semantic search failed:", e); return { results: [] }; }
  },
  async getStats() {
    try {
      var r = await authFetch(WORKER_API + "/api/stats");
      return await r.json();
    } catch (e) { console.error("Stats failed:", e); return {}; }
  },
  async routeToTab(payload) {
    try {
      await authFetch(WORKER_API + "/api/route", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
// v4.28.0 - GDPR Artes Tab + Vercel Skills Audit: DeviceContext, aria-labels, semantic buttons, URL hash, useMemo, focus-visible, reduced-motion
...
// v4.31.0 - Robuuste analyse: Jina Reader fallback, AI kennis fallback, browser user-agent, settings race condition fix
// v4.30.0 - Fix analyse: volledige pagina-tekst scrapen ipv alleen OG description, werkt nu voor artikelen/blogs/Medium
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 10 }}>
        {filtered.map(g => (
          <a key={g.name + g.type} href={`https://infranodus.com/Franky%20DSVD/${g.name}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block" }}>
...
// v4.28.0 — Cross-tab search, semantic matching, gerelateerde items
// ═══════════════════════════════════════════════════════════════════════════════
...
function DumpBar() {
  ...
  useEffect(function() {
    ...
    api.getDump().then(function(data) {
      ...
      setItems(function(local) {
        ...
        var allById = {};
        local.forEach(function(i) { allById[i.id] = Object.assign({}, i); });
        cloudItems.forEach(function(ci) {
          if (!allById[ci.id]) { allById[ci.id] = ci; }
            ...
            if (ci.extraAnalyses && !li.extraAnalyses) li.extraAnalyses = ci.extraAnalyses;
            if (ci.routedTo && !li.routedTo) li.routedTo = ci.routedTo;
            allById[ci.id] = li;
          }
        });
        var merged = Object.keys(allById).map(function(k) { return allById[k]; });
        if (merged.length > 0) { api.saveDump(merged, device); }
        return merged;
      });
      if (data && data.updated) setLastSync(data.updated);
    }).catch(function() {});

    // Auto-refresh elke 30 seconden voor nieuwe iPhone dumps
    var interval = setInterval(function() {
      api.getDump().then(function(data) {
        ...
        setItems(function(local) {
          ...
          var hasNew = cloudItems.some(function(ci) { return !localIds[ci.id]; });
          if (!hasNew) return local;
          var allById = {};
          local.forEach(function(i) { allById[i.id] = Object.assign({}, i); });
          cloudItems.forEach(function(ci) {
            if (!allById[ci.id]) { allById[ci.id] = ci; }
          ...
          return Object.keys(allById).map(function(k) { return allById[k]; });
        });
  ...
  return (
    <div style={{ background: "#0a1a1a", border: "1px solid #0f766e55", borderRadius: 12 }}>
      ...
      {open && items.length > 0 && (
        <div style={{ borderTop: "1px solid #0f766e33", padding: 12, display: "grid", gridTemplateColumns: isPhone ? "1fr" : "1fr 1fr", gap: 10 }}>
            ...
            return (
              <div key={item.id} style={{ background: "#1e1e30", border: "1px solid #2d3748", borderRadius: 10, padding: 14, borderLeft: "3px solid " + c }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 6 }}>
                  <span style={{ fontSize: 16 }}>{item.icon || icons[item.type]} <span style={{ fontSize: 13, color: c, fontWeight: 700 }}>{item.type}</span>{item.analyzed_at && <span title={"Geanalyseerd op " + new Date(item.analyzed_at).toLocaleString("nl-BE")} style={{ fontSize: 11, color: "#4b5563", marginLeft: 8, cursor: "help" }}>✅</span>}</span>
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                    {item.analysis && <button title="Stel een extra vraag over deze analyse — je originele opmerking blijft behouden" onClick={function() { setReanalyzeId(reanalyzeId === item.id ? null : item.id); setRePrompt(""); setRouteId(null); }} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 5, border: "1px solid #3b82f644", background: reanalyzeId === item.id ? "#3b82f633" : "#3b82f611", color: "#3b82f6", cursor: "pointer", fontWeight: 700 }}>🎯 Re-analyze</button>}
                    {item.analysis && <button title="Stuur deze analyse door naar een ander tabblad (SDK-HRM, Knowledge, Crypto, etc.)" onClick={function() { setRouteId(routeId === item.id ? null : item.id); setReanalyzeId(null); }} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 5, border: "1px solid #f59e0b44", background: routeId === item.id ? "#f59e0b33" : "#f59e0b11", color: "#f59e0b", cursor: "pointer", fontWeight: 700 }}>📤 Route</button>}
                    {item.analysis && <button title="Wis analyse volledig — daemon heranalyseert met yt-dlp transcript" onClick={function() { setItems(function(prev) { return prev.map(function(i) { return i.id === item.id ? Object.assign({}, i, { analysis: null, analyzed: false, analyzed_by: null, analyzed_at: null, extraAnalyses: null }) : i; }); }); setPushCount(function(n) { return n + 1; }); }} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 5, border: "1px solid #a855f744", background: "#a855f711", color: "#a855f7", cursor: "pointer", fontWeight: 700 }}>♻️</button>}
                    <button title={item.analysis ? "Opnieuw analyseren met server-side scrape" : "Start AI-analyse van deze content"} onClick={function() { analyzeItem(item.id); }} disabled={item.analyzing} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 5, border: "1px solid #14b8a644", background: "#14b8a611", color: item.analyzing ? "#6b7280" : "#14b8a6", cursor: item.analyzing ? "wait" : "pointer", fontWeight: 700 }}>{item.analyzing ? "⏳..." : item.analysis ? "🔄 Opnieuw" : "🔍 Analyseer"}</button>
                    <button title={item.pinned ? "Losmaken — item staat nu vastgepind bovenaan" : "Vastpinnen — houd dit item bovenaan"} onClick={function() { togglePin(item.id); }} style={{ fontSize: 14, border: "none", background: "none", color: item.pinned ? "#fbbf24" : "#4b5563", cursor: "pointer", padding: 2 }}>{item.pinned ? "📌" : "📍"}</button>
                    <button title="Verwijder dit item" onClick={function() { deleteItem(item.id); }} style={{ fontSize: 14, border: "none", background: "none", color: "#ef4444", cursor: "pointer", padding: 2 }}>🗑️</button>
                  </div>
                </div>
                {/* Enriched card: thumbnail + titel + auteur (YouTube, Instagram, Twitter, articles, links) */}
                {(item.title || item.thumbnail) && item.content && item.content.indexOf("http") === 0 ? (
                ...
                {item.analysis && <div style={{ fontSize: 12, color: "#22c55e", marginTop: 8, padding: "8px 10px", background: "#052e1644", borderRadius: 6, borderLeft: "3px solid #22c55e44", lineHeight: 1.4, maxHeight: isPhone ? "none" : 140, overflowY: "auto" }}>{item.analysis}</div>}
                ...
                {reanalyzeId === item.id && (
                  <div style={{ marginTop: 8, background: "#1a1a2e", borderRadius: 8, padding: 10, border: "1px solid #3b82f633" }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button onClick={function() { reanalyzeItem(item.id, rePrompt); }} disabled={item.analyzing} style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #3b82f6", background: "#3b82f622", color: "#3b82f6", fontSize: 13, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>{item.analyzing ? "⏳" : rePrompt.trim() ? "🎯 Extract" : "🔄 Opnieuw"}</button>
                ...
                <div style={{ fontSize: 11, color: "#4b5563", marginTop: 6 }}>{new Date(item.created).toLocaleDateString("nl-BE", { day: "numeric", month: "short", year: "numeric" }) + " — " + new Date(item.created).toLocaleTimeString("nl-BE", { hour: "2-digit", minute: "2-digit" })}</div>
              </div>
            );
          })}
        </div>
...
// ==================== TO DO TAB ====================
// v4.28.0 - Simpele cross-device takenlijst
function TodoList() {
  const { isPhone, S } = useDevice();
  ...
  const ALL_OPTIONS = ['CCC', 'SDK-HRM', 'Crypto', 'Econation', 'BFW', 'Ecosystem', 'Prompt Input', 'Settings', 'Issues', 'Memory', 'Git', 'Versions', 'Activity', 'Staging', 'Cross-Sync', 'InfraNodus', 'Agents', 'Knowledge', 'Updates', 'OpenClaw', 'Benchmarks', 'Revenue', 'Use Cases', 'All Tools', 'GDPR Artes', 'Advisor', 'To Do\'s'];
  const DEFAULT_PROJECTS = ['CCC', 'SDK-HRM', 'Crypto', 'Econation', 'BFW'];
  const [customProjects, setCustomProjects] = useState(() => {
    try { return JSON.parse(localStorage.getItem('ccc-todo-projects') || '[]'); } catch { return []; }
  });
  const PROJECTS = [...DEFAULT_PROJECTS, ...customProjects];
  const availableToAdd = ALL_OPTIONS.filter(o => !PROJECTS.includes(o));

  const addProject = (name) => {
    if (!name || PROJECTS.includes(name)) return;
    const updated = [...customProjects, name];
    setCustomProjects(updated);
    localStorage.setItem('ccc-todo-projects', JSON.stringify(updated));
  ...
  const API = (JSON.parse(localStorage.getItem("ccc-settings") || "{}")).workerApi || "https://claude-control-center.franky-f29.workers.dev";

  const fetchTodos = useCallback(async () => {
    try {
      const r = await authFetch(API + "/api/todos?status=" + (showDone ? "all" : "open"));
      const data = await r.json();
      setTodos(data.todos || []);
    } catch(e) { console.error("Todos fetch:", e); }
    setLoading(false);
  }, [API, showDone]);
  ...
  const addTodo = async () => {
    ...
    try {
      await authFetch(API + "/api/todos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: newText.trim(), project: newProject || null }) });
      setNewText('');
      await fetchTodos();
    } catch(e) { console.error("Add todo:", e); }
  };

  const toggleDone = async (id, currentStatus) => {
    const next = currentStatus === 'done' ? 'open' : 'done';
    try {
      await authFetch(API + "/api/todos/update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status: next }) });
  ...
  const saveNotes = async (id) => {
    try {
      await authFetch(API + "/api/todos/update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, notes: noteText }) });
      setTodos(prev => prev.map(t => t.id === id ? { ...t, notes: noteText } : t));
  ...
  const deleteTodo = async (id) => {
    try {
      await authFetch(API + "/api/todos?id=" + id, { method: "DELETE" });
  ...
  const updateProject = async (id, project) => {
    try {
      await authFetch(API + "/api/todos/update", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, project: project || null }) });
      setTodos(prev => prev.map(t => t.id === id ? { ...t, project } : t));
      setEditingProject(null);
    } catch(e) { console.error("Update project:", e); }
  };

  const filtered = filterProject ? todos.filter(t => filterProject === '' || t.project === filterProject) : todos;
  const openCount = filtered.filter(t => t.status === 'open').length;
  const doneCount = filtered.filter(t => t.status === 'done').length;

  return (
    <div style={{ padding: S.containerPad, maxWidth: 700, margin: "0 auto" }}>
      <div style={{ marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#fbbf24" }}>📝 To Do's</h2>
        <div style={{ fontSize: 12, color: "#4b5563", marginTop: 2 }}>{openCount} open{doneCount > 0 ? ` • ${doneCount} afgerond` : ''} • Cross-device via D1</div>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <input value={newText} onChange={e => setNewText(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTodo()} placeholder="Wat wil je onthouden of doen?" style={{ flex: 1, padding: isPhone ? "14px 16px" : "10px 14px", borderRadius: 10, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: isPhone ? 16 : 13, outline: "none", boxSizing: "border-box" }} />
        <button onClick={addTodo} disabled={!newText.trim()} style={{ padding: "10px 16px", borderRadius: 10, border: "1px solid #fbbf2444", background: "#fbbf2422", color: "#fbbf24", fontSize: 13, fontWeight: 700, cursor: newText.trim() ? "pointer" : "default", opacity: newText.trim() ? 1 : 0.4, whiteSpace: "nowrap" }}>💾</button>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center", flexWrap: "wrap", position: "relative" }}>
        <span style={{ fontSize: 11, color: "#6b7280" }}>Nieuw:</span>
        {['', ...PROJECTS].map(p => (
          <button key={p} onClick={() => setNewProject(p)} style={{ fontSize: 10, padding: "3px 8px", borderRadius: 6, border: "1px solid " + (newProject === p ? "#fbbf2466" : "#2d3748"), background: newProject === p ? "#fbbf2422" : "transparent", color: newProject === p ? "#fbbf24" : "#6b7280", cursor: "pointer" }}>
            {p || 'Geen'}
            {p && customProjects.includes(p) && <span onClick={(e) => { e.stopPropagation(); removeProject(p); }} style={{ marginLeft: 4, fontSize: 8, color: "#ef444488", cursor: "pointer" }} title="Verwijder project">x</span>}
          </button>
        ))}
        {availableToAdd.length > 0 && <button onClick={() => setAddingProject(!addingProject)} style={{ fontSize: 10, padding: "3px 8px", borderRadius: 6, border: "1px dashed " + (addingProject ? "#22c55e" : "#4b5563"), background: addingProject ? "#22c55e11" : "transparent", color: addingProject ? "#22c55e" : "#4b5563", cursor: "pointer" }} title="Project toevoegen">+</button>}
        {addingProject && (
          <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: 4, background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 8, padding: 8, zIndex: 10, display: "flex", flexWrap: "wrap", gap: 4 }}>
            {availableToAdd.map(opt => (
              <button key={opt} onClick={() => addProject(opt)} style={{ fontSize: 9, padding: "3px 8px", borderRadius: 4, border: "1px solid #22c55e44", background: "#22c55e08", color: "#22c55e", cursor: "pointer", whiteSpace: "nowrap" }}>{opt}</button>
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {['', ...PROJECTS].map(p => (
          <button key={p} onClick={() => setFilterProject(filterProject === p ? '' : p)} style={{ fontSize: 10, padding: "3px 8px", borderRadius: 6, border: "1px solid " + (filterProject === p ? "#a855f766" : "#2d3748"), background: filterProject === p ? "#a855f722" : "transparent", color: filterProject === p ? "#a855f7" : "#6b7280", cursor: "pointer", fontWeight: filterProject === p ? 700 : 400 }}>{p || 'Alle'}</button>
        ))}
        <div style={{ marginLeft: "auto" }}>
          <button onClick={() => setShowDone(!showDone)} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: "1px solid #2d3748", background: showDone ? "#22c55e11" : "transparent", color: showDone ? "#22c55e" : "#6b7280", cursor: "pointer" }}>{showDone ? "✓ Incl. afgeronde" : "Toon afgeronde"}</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 40, color: "#4b5563" }}>{filterProject ? `Geen to do's voor ${filterProject}.` : "Geen to do's. Typ iets hierboven."}</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {filtered.map(t => (
            <div key={t.id} style={{ background: "#1a1a2e", border: "1px solid " + (t.status === 'done' ? "#22c55e33" : "#2d3748"), borderRadius: 10, padding: "10px 14px", opacity: t.status === 'done' ? 0.6 : 1 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <button onClick={() => toggleDone(t.id, t.status)} style={{ width: 22, height: 22, borderRadius: 6, border: "2px solid " + (t.status === 'done' ? "#22c55e" : "#4b5563"), background: t.status === 'done' ? "#22c55e22" : "transparent", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 1, fontSize: 12, color: "#22c55e" }} title={t.status === 'done' ? "Heropen" : "Afronden"}>{t.status === 'done' ? '✓' : ''}</button>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, color: t.status === 'done' ? "#6b7280" : "#e5e5e5", fontWeight: 600, textDecoration: t.status === 'done' ? "line-through" : "none", lineHeight: 1.4 }}>{t.text}</div>
                  {t.notes && editingNotes !== t.id && (
                    <div onClick={() => { setEditingNotes(t.id); setNoteText(t.notes || ''); }} style={{ fontSize: 12, color: "#9ca3af", marginTop: 4, padding: "4px 8px", background: "#0d0d1a", borderRadius: 6, cursor: "pointer", lineHeight: 1.4, whiteSpace: "pre-wrap" }}>{t.notes}</div>
                  )}
                  {editingNotes === t.id && (
                    <div style={{ marginTop: 4 }}>
                      <textarea value={noteText} onChange={e => setNoteText(e.target.value)} rows={2} style={{ width: "100%", padding: "6px 8px", borderRadius: 6, border: "1px solid #2d3748", background: "#0d0d1a", color: "#e5e5e5", fontSize: 12, outline: "none", resize: "vertical", boxSizing: "border-box" }} autoFocus />
                      <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                        <button onClick={() => saveNotes(t.id)} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 4, border: "1px solid #22c55e44", background: "#22c55e11", color: "#22c55e", cursor: "pointer" }}>Opslaan</button>
                        <button onClick={() => setEditingNotes(null)} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 4, border: "1px solid #2d3748", background: "transparent", color: "#6b7280", cursor: "pointer" }}>Annuleer</button>
                      </div>
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center", flexWrap: "wrap", position: "relative" }}>
                    <span onClick={() => setEditingProject(editingProject === t.id ? null : t.id)} style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: t.project ? "#fbbf2422" : "#2d374822", color: t.project ? "#fbbf24" : "#6b7280", border: "1px solid " + (t.project ? "#fbbf2433" : "#2d3748"), cursor: "pointer" }} title="Klik om project te wijzigen">{t.project || '+'}</span>
                    {editingProject === t.id && (
                      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                        {['', ...PROJECTS].map(p => (
                          <button key={p} onClick={() => updateProject(t.id, p)} style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, border: "1px solid " + ((t.project || '') === p ? "#a855f766" : "#2d3748"), background: (t.project || '') === p ? "#a855f722" : "#0d0d1a", color: (t.project || '') === p ? "#a855f7" : "#9ca3af", cursor: "pointer" }}>{p || '—'}</button>
                        ))}
                      </div>
                    )}
                    <span style={{ fontSize: 10, color: "#4b5563" }}>{new Date(t.created_at + 'Z').toLocaleDateString('nl-BE', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  {editingNotes !== t.id && (
                    <button onClick={() => { setEditingNotes(t.id); setNoteText(t.notes || ''); }} style={{ fontSize: 11, padding: "2px 6px", borderRadius: 4, border: "1px solid #2d3748", background: "transparent", color: "#6b7280", cursor: "pointer" }} title="Notitie toevoegen/bewerken">📝</button>
                  )}
                  <button onClick={() => deleteTodo(t.id)} style={{ fontSize: 11, padding: "2px 6px", borderRadius: 4, border: "1px solid #ef444433", background: "transparent", color: "#ef4444", cursor: "pointer" }} title="Verwijder">🗑️</button>
                </div>
              </div>
            </div>
          ))}
...
// ==================== PROMPT INPUT TAB ====================
// v4.28.0 - Directives: togglebare prompt-context items voor SessionStart hook
function PromptInput() {
  ...
  const [newDir, setNewDir] = useState({ name: '', category: 'custom', description: '', content: '', icon: '📌', color: '#8b5cf6' });
  const API = (JSON.parse(localStorage.getItem("ccc-settings") || "{}")).workerApi || "https://claude-control-center.franky-f29.workers.dev";

  const fetchDirectives = useCallback(async () => {
    try {
      const r = await authFetch(API + "/api/directives");
      const data = await r.json();
      setDirectives(data.directives || []);
    } catch(e) { console.error("Directives fetch:", e); }
    setLoading(false);
  }, [API]);
  ...
  const toggle = async (id, active) => {
    setSaving(true);
    try {
      await authFetch(API + "/api/directives/toggle", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, active }) });
      setDirectives(prev => prev.map(d => d.id === id ? { ...d, active: active ? 1 : 0 } : d));
    } catch(e) { console.error("Toggle:", e); }
    setSaving(false);
  };

  const addDirective = async () => {
    if (!newDir.name || !newDir.content) return;
    setSaving(true);
    try {
      await authFetch(API + "/api/directives", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...newDir, active: false }) });
      setShowAdd(false);
      setNewDir({ name: '', category: 'custom', description: '', content: '', icon: '📌', color: '#8b5cf6' });
      await fetchDirectives();
    } catch(e) { console.error("Add:", e); }
    setSaving(false);
  };

  const deleteDirective = async (id) => {
    setSaving(true);
    try {
      await authFetch(API + "/api/directives?id=" + id, { method: "DELETE" });
      setDirectives(prev => prev.filter(d => d.id !== id));
    } catch(e) { console.error("Delete:", e); }
    setSaving(false);
  };

  const categories = [...new Set(directives.map(d => d.category))].sort();
  const activeCount = directives.filter(d => d.active).length;
  const catIcons = { skills: '⚡', project: '📂', language: '🌍', security: '🔒', coding: '✂️', custom: '📌' };

  return (
    <div style={{ padding: S.containerPad, maxWidth: 900, margin: "0 auto" }}>
      {/* Header */}
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#a855f7" }}>🎯 Prompt Input</h2>
          <div style={{ fontSize: 12, color: "#4b5563", marginTop: 2 }}>{activeCount} actief van {directives.length} directieven • SessionStart hook injecteert actieve items</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setShowAdd(!showAdd)} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #a855f744", background: "#a855f711", color: "#a855f7", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{showAdd ? "✕ Annuleer" : "➕ Nieuw Directief"}</button>
          <button onClick={fetchDirectives} disabled={loading} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #3b82f644", background: "#3b82f611", color: "#3b82f6", fontSize: 12, fontWeight: 700, cursor: loading ? "wait" : "pointer" }}>🔄</button>
        </div>
      </div>

      {/* Active summary banner */}
      {activeCount > 0 && (
        <div style={{ background: "#22c55e11", border: "1px solid #22c55e33", borderRadius: 10, padding: 12, marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#22c55e", marginBottom: 6 }}>🟢 Actieve Directieven — worden geïnjecteerd bij volgende sessie</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {directives.filter(d => d.active).map(d => (
              <span key={d.id} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 6, background: d.color + "22", color: d.color, border: "1px solid " + d.color + "44", fontWeight: 600 }}>{d.icon} {d.name}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {showAdd && (
        <div style={{ background: "#1a1a2e", border: "1px solid #a855f744", borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#a855f7", marginBottom: 12 }}>➕ Nieuw Directief Aanmaken</div>
          <div style={{ display: "grid", gridTemplateColumns: isPhone ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Naam</label>
              <input value={newDir.name} onChange={e => setNewDir({...newDir, name: e.target.value})} placeholder="bv. Mijn Custom Rule" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Categorie</label>
              <select value={newDir.category} onChange={e => setNewDir({...newDir, category: e.target.value})} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Beschrijving (kort)</label>
            <input value={newDir.description} onChange={e => setNewDir({...newDir, description: e.target.value})} placeholder="Korte uitleg wat dit directief doet" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Content (wordt geïnjecteerd in prompt)</label>
            <textarea value={newDir.content} onChange={e => setNewDir({...newDir, content: e.target.value})} rows={5} placeholder="De instructies/context die Claude krijgt wanneer dit directief actief is..." style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "monospace" }} />
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div>
              <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Icon</label>
              <input value={newDir.icon} onChange={e => setNewDir({...newDir, icon: e.target.value})} style={{ width: 60, padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 16, outline: "none", textAlign: "center" }} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 3 }}>Kleur</label>
              <input type="color" value={newDir.color} onChange={e => setNewDir({...newDir, color: e.target.value})} style={{ width: 50, height: 36, borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", cursor: "pointer" }} />
            </div>
            <div style={{ flex: 1, display: "flex", alignItems: "flex-end" }}>
              <button onClick={addDirective} disabled={saving || !newDir.name || !newDir.content} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #22c55e44", background: "#22c55e22", color: "#22c55e", fontSize: 13, fontWeight: 700, cursor: saving ? "wait" : "pointer", opacity: (!newDir.name || !newDir.content) ? 0.4 : 1 }}>💾 Opslaan</button>
            </div>
          </div>
        </div>
      )}

      {/* Directive cards per category */}
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: "#6b7280" }}>Laden...</div>
      ) : (
        categories.map(cat => (
          <div key={cat} style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#9ca3af", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>
              {catIcons[cat] || '📌'} {cat} ({directives.filter(d => d.category === cat).length})
            </div>
            <div style={{ display: "grid", gridTemplateColumns: isPhone ? "1fr" : "1fr 1fr", gap: 10 }}>
              {directives.filter(d => d.category === cat).map(d => (
                <div key={d.id} style={{ background: "#1a1a2e", border: "1px solid " + (d.active ? d.color + "66" : "#2d3748"), borderRadius: 10, padding: 14, transition: "border-color 0.2s", position: "relative" }}>
                  {/* Header with toggle */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
                      <span style={{ fontSize: 20 }}>{d.icon}</span>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: d.active ? "#e5e5e5" : "#6b7280" }}>{d.name}</div>
                        {d.description && <div style={{ fontSize: 11, color: "#4b5563", marginTop: 1 }}>{d.description}</div>}
                      </div>
                    </div>
                    <button onClick={() => toggle(d.id, !d.active)} disabled={saving} style={{ width: 48, height: 26, borderRadius: 13, border: "none", background: d.active ? d.color : "#374151", cursor: saving ? "wait" : "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }} title={d.active ? "Uitschakelen" : "Inschakelen"}>
                      <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#fff", position: "absolute", top: 3, left: d.active ? 25 : 3, transition: "left 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.3)" }} />
                    </button>
                  </div>
                  {/* Content preview */}
                  <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.5, maxHeight: 60, overflow: "hidden", background: "#0d0d1a", borderRadius: 6, padding: "6px 8px", fontFamily: "monospace", whiteSpace: "pre-wrap", marginBottom: 6 }}>
                    {d.content.substring(0, 200)}{d.content.length > 200 ? "..." : ""}
                  </div>
                  {/* Footer */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", gap: 4 }}>
                      <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: d.color + "22", color: d.color, border: "1px solid " + d.color + "33" }}>{d.category}</span>
                      <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: "#1e1e30", color: "#4b5563", border: "1px solid #2d3748" }}>{d.source}</span>
                    </div>
                    {d.source !== 'preset' && (
                      <button onClick={() => deleteDirective(d.id)} style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, border: "1px solid #ef444433", background: "transparent", color: "#ef4444", cursor: "pointer" }} title="Verwijder directief">🗑️</button>
                    )}
                  </div>
                </div>
              ))}
      ...
      {/* Info footer */}
      <div style={{ background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 10, padding: 12, marginTop: 8, textAlign: "center" }}>
        <div style={{ fontSize: 11, color: "#4b5563" }}>Actieve directieven worden geïnjecteerd via SessionStart hook bij elke nieuwe Claude Code sessie.</div>
        <div style={{ fontSize: 11, color: "#4b5563", marginTop: 4 }}>Directieven zijn cross-device — opgeslagen in Cloudflare D1, beschikbaar op MM4, MBA, MM2.</div>
      </div>
    </div>
  );
}
...
// ==================== SETTINGS TAB ====================
// v4.28.0 - CCC Settings: features aan/uit, sync config, service status
function CCCSettings() {
  var _dev = useDevice(), isPhone = _dev.isPhone, S = _dev.S;
  var _s = useState(function() { try { return JSON.parse(localStorage.getItem("ccc-settings") || "{}"); } catch(e) { return {}; } });
  ...
  var _sL = useState(false); var loading = _sL[0]; var setLoading = _sL[1];
  var fetchStatus = function() { setLoading(true); Promise.all([fetch(cfg.workerApi+"/api/health").then(function(r){return r.json()}).catch(function(){return null}), authFetch(cfg.workerApi+"/api/stats").then(function(r){return r.json()}).catch(function(){return null})]).then(function(res){setHealth(res[0]);setStats(res[1]);setLoading(false)}); };
  useEffect(function() { fetchStatus(); }, []);

  var services = [
    { label: "D1 Database", key: "d1", icon: "🗃️" },
    { label: "Vectorize", key: "vectorize", icon: "🧲" },
    { label: "Queue", key: "queue", icon: "📨" },
    { label: "AI Embeddings", key: "ai", icon: "🧠" },
  ];
  var toggles = [
    { id: "dumpEnabled", label: "Dump Bar", desc: "Toon de Dump bar bovenaan voor content capture" },
    { id: "dumpAutoSync", label: "Auto-Sync", desc: "Sync dump items automatisch met Cloudflare bij laden" },
    { id: "dumpAnalysisDaemon", label: "Analyse Daemon", desc: "Lokale daemon analyseert nieuwe dump items automatisch" },
    { id: "universalSearch", label: "Universal Search", desc: "Zoekbalk boven de tabs — zoekt over D1 + Vectorize" },
    { id: "knowledgeSync", label: "Knowledge Sync", desc: "Sync claude-mem observations + MEMORY.md naar cloud" },
    { id: "vectorizeEnabled", label: "Vectorize", desc: "Automatisch embeddings genereren voor semantic search" },
    { id: "advisorEnabled", label: "AI Advisor", desc: "Toon de AI Advisor bar onder de dump" },
    { id: "activityLogging", label: "Activity Logging", desc: "Log alle acties naar D1 + KV" },
    { id: "deviceSelector", label: "Device Selector", desc: "Toon device knoppen in header" },
  ];
  var statCards = stats && stats.dumps ? [
    { label: "Dump Items", val: stats.dumps.total || 0, sub: (stats.dumps.analyzed || 0) + " analyzed", color: "#14b8a6" },
    { label: "Activity Logs", val: (stats.logs && stats.logs.total) || 0, sub: "events tracked", color: "#f59e0b" },
    { label: "Knowledge", val: (stats.knowledge || []).reduce(function(a,k){return a+k.count},0), sub: (stats.knowledge || []).length + " categories", color: "#ec4899" },
    { label: "Vectors", val: (stats.vectors || []).reduce(function(a,v){return a+v.count},0), sub: (stats.vectors || []).length + " types indexed", color: "#8b5cf6" },
  ] : [];

  return (
    <div style={{ padding: S.containerPad, maxWidth: 800, margin: "0 auto" }}>
      {/* Header */}
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#9ca3af" }}>⚙️ Settings</h2>
          <div style={{ fontSize: 12, color: "#4b5563", marginTop: 2 }}>CCC v4.31.0 • Worker v3.3.0</div>
        </div>
      ...
      {/* Cloud Services */}
      {health && (
        <div style={{ background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 12, padding: 14, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#e5e5e5", marginBottom: 10 }}>☁️ Cloudflare Services</div>
          <div style={{ display: "grid", gridTemplateColumns: isPhone ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 8 }}>
            {services.map(function(svc) {
              var on = health.services && health.services[svc.key];
              return (
                <div key={svc.label} style={{ padding: "8px 10px", borderRadius: 8, background: on ? "#22c55e11" : "#ef444411", border: "1px solid " + (on ? "#22c55e33" : "#ef444433"), textAlign: "center" }}>
                  <div style={{ fontSize: 18 }}>{svc.icon}</div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: on ? "#22c55e" : "#ef4444", marginTop: 2 }}>{svc.label}</div>
                  <div style={{ fontSize: 10, color: on ? "#4ade80" : "#f87171" }}>{on ? "Active" : "Offline"}</div>
                </div>
              );
            })}
          </div>
          <div style={{ display: "flex", gap: 16, marginTop: 10, fontSize: 11, color: "#6b7280" }}>
            <span>Worker: v{health.version || "?"}</span>
            <span>Vectors: {health.vectors || 0}</span>
            <span>Region: WEUR/Amsterdam</span>
          </div>
        </div>
      )}
      ...
      {statCards.length > 0 && (
        <div style={{ background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 12, padding: 14, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#e5e5e5", marginBottom: 10 }}>📊 Data Overview</div>
          <div style={{ display: "grid", gridTemplateColumns: isPhone ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 8 }}>
            {statCards.map(function(st) {
              return (
                <div key={st.label} style={{ padding: 10, borderRadius: 8, background: st.color + "11", border: "1px solid " + st.color + "33" }}>
                  <div style={{ fontSize: 22, fontWeight: 800, color: st.color }}>{st.val}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#e5e5e5", marginTop: 2 }}>{st.label}</div>
                  <div style={{ fontSize: 10, color: "#6b7280" }}>{st.sub}</div>
                </div>
              );
            })}
          </div>
      ...
      {/* Feature Toggles */}
      <div style={{ background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 12, padding: 14, marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#e5e5e5", marginBottom: 10 }}>🎛️ Features</div>
        {toggles.map(function(t) {
          var on = cfg[t.id];
          return (
            <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#1e1e30", borderRadius: 8, border: "1px solid #2d3748", marginBottom: 6 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: on ? "#e5e5e5" : "#6b7280" }}>{t.label}</div>
                <div style={{ fontSize: 11, color: "#4b5563", marginTop: 2 }}>{t.desc}</div>
              </div>
              <button onClick={function() { update(t.id, !on); }} style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: on ? "#22c55e" : "#374151", cursor: "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
      ...
      {/* Config */}
      <div style={{ background: "#1a1a2e", border: "1px solid #2d3748", borderRadius: 12, padding: 14, marginBottom: 16 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#e5e5e5", marginBottom: 10 }}>🔧 Configuratie</div>
        <div style={{ marginBottom: 10 }}>
          <label style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 4 }}>Worker API URL</label>
          <input type="text" value={cfg.workerApi} onChange={function(e) { update("workerApi", e.target.value); }} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
        </div>
        <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          <div>
            <label style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 4 }}>Sync Interval (sec)</label>
            <input type="number" value={cfg.syncInterval} onChange={function(e) { update("syncInterval", parseInt(e.target.value) || 120); }} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 4 }}>Analyse Taal</label>
            <select value={cfg.analyseLanguage} onChange={function(e) { update("analyseLanguage", e.target.value); }} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }}>
          ...
          <div>
            <label style={{ fontSize: 12, color: "#9ca3af", fontWeight: 600, display: "block", marginBottom: 4 }}>Max Bullets</label>
            <input type="number" value={cfg.maxAnalyseBullets} min={3} max={10} onChange={function(e) { update("maxAnalyseBullets", parseInt(e.target.value) || 5); }} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #2d3748", background: "#1e1e30", color: "#e5e5e5", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ textAlign: "center", marginTop: 8 }}>
        <button onClick={function() { setSettings({}); localStorage.removeItem("ccc-settings"); }} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid #ef444444", background: "#ef444411", color: "#ef4444", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🔄 Reset naar standaard</button>
        <div style={{ fontSize: 10, color: "#4b5563", marginTop: 6 }}>Settings worden opgeslagen in localStorage — beschikbaar per browser/device</div>
      </div>
    </div>
  );
}
...
export default function ControlCenter() {
  ...
  const [tab, setTab] = useState(() => {
    ...
    return hash && ["ecosystem","prompt","todos","issues","memory","git","versions","activity","staging","sync","infranodus","agents","knowledge","updates","openbot","sdkhrm","benchmarks","crypto","revenue","usecases","alltools","advisor","settings"].includes(hash) ? hash : "ecosystem";
  });
  ...
  const tabs = [
    ...
    { id: "prompt", label: "🎯 Prompt Input", color: "#a855f7", lastUpdated: "10 Feb" },
    { id: "todos", label: "📝 To Do's", color: "#fbbf24", lastUpdated: "10 Feb" },
    { id: "settings", label: "⚙️ Settings", color: "#9ca3af", lastUpdated: "10 Feb" },
  ...
  return (
    ...
    <div style={{ fontFamily: "'SF Pro Text', -apple-system, sans-serif", background: S.bgBody, color: "#e5e5e5", minHeight: "100vh", padding: S.containerPad }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0f0f23, #1a0a2e, #0a1628)", border: "1px solid #2d2a5e", borderRadius: 16, padding: "16px 20px", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div>
            ...
            <div style={{ fontSize: S.smallFont, color: "#6b7280", marginTop: 2 }}>DS2036 — Franky | v4.31.0 | {new Date().toLocaleDateString("nl-BE")}</div>
          </div>
      ...
      {/* UNIVERSAL SEARCH — zoek over alles heen (D1 SQL + Vectorize semantic) */}
      {(JSON.parse(localStorage.getItem("ccc-settings") || "{}").universalSearch !== false) && <UniversalSearch onNavigate={setTab} />}

      {/* DUMP - Altijd zichtbaar bovenaan */}
      {(JSON.parse(localStorage.getItem("ccc-settings") || "{}").dumpEnabled !== false) && <DumpBar />}

      {/* ADVISOR - Prominent bar (always visible) */}
      {(JSON.parse(localStorage.getItem("ccc-settings") || "{}").advisorEnabled !== false) && <AIAdvisor issues={issues} compact={true} onNavigate={setTab} currentDevice={currentDevice} />}
      ...
      <footer style={{ marginTop: 16, padding: S.containerPad, background: S.bgFooter, border: "1px solid #2d3748", borderRadius: 10, textAlign: "center" }}>
        <div style={{ fontSize: S.smallFont, color: "#4b5563" }}>Claude Control Center v4.31.0 • {total} nodes • 21 tabs • Perplexity Intelligence • Device: {currentDevice} • Cloudflare: claude-ecosystem-dashboard.pages.dev</div>
