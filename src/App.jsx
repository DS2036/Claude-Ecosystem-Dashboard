...
function VersionSnapshots() {
  const [snapshots, setSnapshots] = useState([
    { id: 1, name: "v1.0.0 - Initial Dashboard", date: "2026-02-05 17:30", project: "Claude-Ecosystem-Dashboard", type: "manual", commit: "384543d" },
    { id: 2, name: "v2.0.0 - Control Center", date: "2026-02-06 00:55", project: "Claude-Ecosystem-Dashboard", type: "auto", commit: "fd6d490" },
    { id: 3, name: "v3.0.0 - Merged v1+v2", date: "2026-02-06 01:15", project: "Claude-Ecosystem-Dashboard", type: "manual", commit: "current" },
  ]);
  const [newSnapshot, setNewSnapshot] = useState({ name: "", project: "Claude-Ecosystem-Dashboard" });

  const createSnapshot = () => {
    if (!newSnapshot.name.trim()) return;
    setSnapshots(prev => [{ id: Date.now(), name: newSnapshot.name, date: new Date().toLocaleString("nl-BE"), project: newSnapshot.project, type: "manual", commit: "pending" }, ...prev]);
            ...
            <div key={snap.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#1a1a2e", border: "1px solid #374151", borderRadius: 8, padding: 12 }}>
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, background: "#3b82f622", color: "#60a5fa", fontFamily: "monospace" }}>{snap.commit}</span>
                </div>
              </div>
...
function ActivityLog() {
  const [activities] = useState([
    { id: 1, time: "01:15", source: "Chat", mac: "MBA", action: "Dashboard v3 merged", detail: "v1 + v2 combined", type: "deploy" },
    { id: 2, time: "00:55", source: "Chat", mac: "MBA", action: "Dashboard v2 deployed", detail: "Control Center features", type: "deploy" },
    { id: 3, time: "00:35", source: "Chat", mac: "MBA", action: "Memory inject", detail: "Claude Code updated v2.1.32", type: "change" },
    { id: 4, time: "00:30", source: "Chat", mac: "MBA", action: "Cloudflare deploy", detail: "claude-ecosystem-dashboard.pages.dev", type: "deploy" },
    { id: 5, time: "00:25", source: "Chat", mac: "MBA", action: "Git push", detail: "GitHub Actions auto-deploy", type: "git" },
    { id: 6, time: "00:20", source: "Chat", mac: "MBA", action: "File created", detail: "bridge/claude-mem-bridge.py", type: "file" },
    { id: 7, time: "00:15", source: "Chat", mac: "MBA", action: "API Token created", detail: "Cloudflare GitHub Actions", type: "config" },
    { id: 8, time: "23:30", source: "CLI", mac: "MBA", action: "Session start", detail: "Econation project", type: "session" },
    { id: 9, time: "23:00", source: "CLI", mac: "MBA", action: "Brain save", detail: "12 observations stored", type: "memory" },
  ]);
  const typeColors = { change: { bg: "#22c55e22", color: "#4ade80" }, deploy: { bg: "#06b6d422", color: "#22d3ee" }, git: { bg: "#a78bfa22", color: "#c4b5fd" }, file: { bg: "#3b82f622", color: "#60a5fa" }, config: { bg: "#f59e0b22", color: "#fbbf24" }, session: { bg: "#ec489922", color: "#f472b6" }, memory: { bg: "#8b5cf622", color: "#a78bfa" } };
  ...
  return (
    <div style={{ background: "#0f0f0f", border: "1px solid #1f2937", borderRadius: 12, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["All", "Chat", "CLI"].map(f => <button key={f} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #374151", background: f === "All" ? "#1e1b4b" : "#111", color: f === "All" ? "#c4b5fd" : "#6b7280", fontSize: 11, cursor: "pointer" }}>{f}</button>)}
        </div>
      </div>
